 ...
 *
 * Run with: npx tsx scripts/fix-supabase-permissions.js
 */
...
const connectionString =
  'postgres://postgres:6omgc0yPSyUV5ScT@db.xdlqzwrhbtltmqxdrwrs.supabase.co:5432/postgres'
...
async function fixPermissions() {
  ...
  try {
    ...
    if (schemaCheck.rows.length === 0) {
      console.log(
        '❌ Schema "payload" does not exist yet. Run the app first to create it.'
      )
      return
    }
    ...
    for (const row of tables.rows) {
      await client.query(
        `ALTER TABLE payload."${row.tablename}" DISABLE ROW LEVEL SECURITY;`
      )
    ...
    await client.query(
      `GRANT USAGE ON SCHEMA payload TO postgres, anon, authenticated, service_role;`
    )
    console.log('   ✅ Granted USAGE on schema payload')

    // Grant all on all tables
    await client.query(
      `GRANT ALL ON ALL TABLES IN SCHEMA payload TO postgres, anon, authenticated, service_role;`
    )
    console.log('   ✅ Granted ALL on all tables')

    // Grant all on all sequences
    await client.query(
      `GRANT ALL ON ALL SEQUENCES IN SCHEMA payload TO postgres, anon, authenticated, service_role;`
    )
    console.log('   ✅ Granted ALL on all sequences')

    // Set default privileges for future tables
    await client.query(
      `ALTER DEFAULT PRIVILEGES IN SCHEMA payload GRANT ALL ON TABLES TO postgres, anon, authenticated, service_role;`
    )
    await client.query(
      `ALTER DEFAULT PRIVILEGES IN SCHEMA payload GRANT ALL ON SEQUENCES TO postgres, anon, authenticated, service_role;`
    )
    ...
    if (verifyRls.rows.length === 0) {
      ...
      console.log(
        `   ⚠️  ${verifyRls.rows.length} table(s) still have RLS enabled`
      )
    }
    ...
    if (publicMediaCheck.rows.length > 0) {
      ...
      for (const row of publicMediaCheck.rows) {
        await client.query(
          `ALTER TABLE public."${row.tablename}" DISABLE ROW LEVEL SECURITY;`
        )
